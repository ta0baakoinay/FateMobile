...
14      uint8[2] unknown/padding — NOT contiguous with login_id2; do not skip
                                    this when building the packet. Zero-fill it.
16      uint8  sex             (from AC_ACCEPT_LOGIN)
                                total 17 bytes
```
Confirmed at `char_clif.cpp:775` (`chclif_parse_reqtoconnect`): `RFIFOL(fd,10)` (login_id2) then `RFIFOB(fd,16)` (sex) — **not** offset 14. A live-server packet capture (2026-09-14) caught exactly this mistake in an earlier pass of this client's own code: `sex` was written immediately after `login_id2` with no gap, landing at offset 14 instead of 16, so the server read its own zero-padding as `sex`, silently corrupting it to `SEX_FEMALE` regardless of the account's real sex — account_id and both login_ids matched fine, so the failure looked like an opaque, intermittent auth rejection with no clear cause until the actual bytes on the wire were inspected. The lesson: when a struct has a gap documented as "jump from offset X to offset Y", that gap is real and must be filled, not an editing artifact to be closed up.
...
**PIN code note — UPDATE from live testing (2026-09-14): the packet arrives even with `pincode_enabled: no`.** `HC_ACK_PINCODE` (**0x08B9**, 12 bytes: `int16 packetType; uint32 seed; uint32 account_id; uint16 state;`, state enum in `char.hpp:113` — `PINCODE_OK=0, PINCODE_ASK=1, PINCODE_NEW=4, PINCODE_PASSED=7`) was originally documented here as conditional on `pincode_enabled` in `conf/char_athena.conf` (confirmed `no` in this server's shipped config, and `logclif_pincode_start`'s own source is gated on that flag with no `else` branch). Live testing against the actual running server found this packet arriving anyway — sent with `state=0` (`PINCODE_PASSED`) immediately after `CH_SELECT_CHAR`/`CH_MAKE_CHAR`, *before* the real response. The exact trigger wasn't root-caused (candidates: a different code path than `logclif_pincode_start` that isn't config-gated, or the running process simply predates some config change — see the `char_new` staleness finding below, which turned out to be exactly this kind of stale-in-memory-config issue). **Practical handling, now implemented in `CharServerClient.kt`**: treat `0x08B9` as an unsolicited, ignorable packet that can precede the real response to `CH_SELECT_CHAR`, `CH_MAKE_CHAR`, or `CH_DELETE_CHAR` — consume its 10-byte body and read the next opcode for the actual answer, rather than treating it as the terminal response or as a stream desync. Still no pincode *entry* UI (nothing here requires one — `state` observed live is always `PINCODE_PASSED`), but the client no longer hangs/errors when the packet shows up.
...
**Found only by live-testing (2026-09-14), not by static source analysis — an extra packet actually precedes `ZC_ACCEPT_ENTER`**: `ZC_EXTEND_BODYITEM_SIZE`, opcode **`0x0B18`**, 4 bytes (`int16 packetType; int16 expansionSize;`, `src/map/packets_struct.hpp:3816`, sent by `clif_inventory_expansion_info()`). It reports the account's inventory-slot expansion and requires `sd` to already be loaded, so it fires as part of the same "player is authenticating on this map-server" window as `ZC_ACCEPT_ENTER` — just before it, not after. This was missed by the original opcode-by-opcode source trace because nothing pointed to `clif_inventory_expansion_info()` as relevant during the initial `CZ_ENTER`/`clif_authok` research; it only surfaced when the real handshake was captured end-to-end against the live server. **Consequence, and the actual lesson**: the "read the session echo, then expect exactly one more packet" model in this document was incomplete — this connect window can carry additional packets beyond the ones a static trace happens to find, the same way §4.2's char-list push turned out to be four packets instead of one. `MapServerClient.kt` now consumes and skips `0x0B18` rather than treating it as a stream desync. Treat any "expect packet X next" claim in this document as a *lower bound*, not a guarantee of completeness, until it's been exercised against a live server.
